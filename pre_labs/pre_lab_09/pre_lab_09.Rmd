...
https://www.datawrapper.de/_/VaxQC/
...
md_gov_county <- md_gov_county |>
  group_by(County) |>
  mutate(
    total_votes = sum(Cox + Moore + Lashar + Wallace + Harding, na.rm = TRUE),  
    moore_percentage = (Moore / total_votes) * 100 
  ) |>
  ungroup()
write.csv(md_gov_county, "data/md_gov_county_updated.csv", row.names = FALSE)
```
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer** This now shows the zoomed in map
...
##It changed it to just include the county with the Moore percentage of the total
...
https://www.datawrapper.de/_/dSYg9/