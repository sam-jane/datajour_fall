...
**Answer** It has added a column to the pre-existing dats
...
**Answer** We arranged the ordering to make it so the greatest percentages went first. This made it so the top amount of people who voted on election day, their county showed first.
...
**Answer** I think this is more interesting because of majority of constituents still vote in-person on election day. Sorting it by which counties had the least amount of people who voted over the mail, and then compare it to how those people voted and which party.
...
maryland_winred_categories <- maryland_winred_contributions |>
  ...
  summarize(contribs=n(), total = sum(amount))
```
...
**Answer** The highest amount given was less than $100.
...
**Answer:** It made it so that the function was a <date> instead of a char which changed it to make it the ideal forumla, year, date, day. :).
...
**Answer** It's showing which dates had the most contributions. But these are techically one day of the month, on the first of the each month, rather than the entire month? We need a new way to make it so it's the month as the entirety.
