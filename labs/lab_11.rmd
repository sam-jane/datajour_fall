...
board_results <- board_results[[9]] |> 
  slice(-9, -7) |> 
  select(Name, `Mail-In Ballot`, `Early Voting`, `Election Day`, Percentage) |> 
  mutate(
...
board_results |>
  ...
  coord_flip() +
  labs(
    title = "Republicans take one seat on the Frederick County Board of Education",
...
**A2** 
##John King in the election for governor
...
most_recent_campaign <- press_releases_df |> 
...
**A3** It would be good to see if there were any way we could a topic, or a sentence line that describes what the press release would be about. We could also do a story about how many times a press release eventually contradicts itself with language. We could see what days of the week are units most likley to release press releases.
...
cardin_titles <- cardin_results |>
  html_elements("title") |>
  html_text()
titles_TT <- tibble(Title = cardin_titles)
titles_fixed <- titles_TT |>
  filter(!str_detect(Title, "Read More")) 
```


```{r}
dates_cardin <- cardin_results |>
  html_elements("h5") |>
  html_text()
dates_cardin

filtered_dates <- tibble(
  Date = dates_cardin) |>
  mutate(Date = mdy(Date))
```
...
```{r}
Combined_cardin <- bind_cols(filtered_dates, titles_fixed, filtered_urls)
```