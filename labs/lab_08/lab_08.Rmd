...
```{r}
# load the data
md_absentee_returned <- read_csv("data/md_absentee_ballots.csv")
md_date_absentee <- read_csv("data/md_absentee_ballots_date.csv")
```

```{r}
# load the data
md_absentee_returned_clean <- md_absentee_returned |>
  clean_names() 
md_date__clean <- md_date_absentee |>
  clean_names() 
```
...
**A1.** The highest is Worcester

```{r}
md_absentee_returned_clean <- md_absentee_returned_clean |>
mutate(PERCENTAGE = (total_received / total_sent)*100)
maryland_top_counties <- md_absentee_returned_clean |>
...
maryland_top_counties |>
  ggplot() +
  geom_bar(aes(x=county_name, weight=PERCENTAGE))
```
...
maryland_top_counties |>
  ...
  coord_flip() + 
labs(
    title="Maryland's biggest counties don't have high return ballot rate",
    x = "Returned ballots %",
    y = "County",
...
**A2.** Baltimore City, Montgomery, Prince George's, Frederick, Baltimore County, Charles, Howard, Cecil, Anne Arundel and Carroll had the lowest rates of returned Democratic ballots.

```{r}
md_absentee_returned_clean <- md_absentee_returned_clean |>
mutate(Dems_PERCENTAGE = (dem_received / dem_sent)*100) |>
mutate(Reps_PERCENTAGE = (rep_received / rep_sent)*100) 
```

```{r}
parties_returned <- md_absentee_returned_clean |>
  arrange(Dems_PERCENTAGE, Reps_PERCENTAGE) |>
  select(county_name,Dems_PERCENTAGE, Reps_PERCENTAGE) |>
  head(10)
```
...
parties_returned |>
  ggplot() +
  geom_bar(aes(x=reorder(county_name,Dems_PERCENTAGE), weight=Dems_PERCENTAGE)) +
  coord_flip() + 
labs(
    title="Baltimore City has the lowest returned ballot rate in Maryland",
    x = "Returned Democratic ballots %",
    y = "County",
    caption = "source: Maryland State Board of Elections"
  )
theme_minimal()
...
**A3** That Democratic ballots have surged way ahead of other parties in October

```{r}
Frederick_county <- md_date__clean |>
...
geom_line(aes(x= date, y = oth_received, color = "Oth Received")) +
labs(title = "Voters Over October", y = "Received Voters", x = "Date") 
```
```
```
...
**A4.** There are some counties that are definitely ahead in the ballots they've returned compared to counties that they sent.I think it's also somewhat hard to read this graph, because there's a lack of clarity at the bottom. I struggled with how to seperate those out. 
```{r}
